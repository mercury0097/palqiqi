...
#define HAND_HOME_POSITION 170      // 左手放下位置
#define RIGHT_HAND_HOME_POSITION 35 // 右手放下位置（安装方向相反）
...
void Otto::Jump(float steps, int period) {
  int homes[SERVO_COUNT] = {
      90, 90, 90, 90, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  
  // 阶段1: 准备蓄力（下蹲）- 使用 EASE_IN_OUT 平滑下蹲，30%时间
  int crouch[SERVO_COUNT] = {
      90, 90, 120, 60, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  MoveServosWithEase(period * 0.30, crouch, EASE_IN_OUT);
  
  // 阶段2: 上跳（有力但不急）- 使用 EASE_OUT，15%时间
  int up[SERVO_COUNT] = {
      90, 90, 150, 30, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  ...
  int land[SERVO_COUNT] = {
      90, 90, 110, 70, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
...
void Otto::Walk(float steps, int period, int dir, int amount) {
  ...
  // 偏移：脚部微微踮起，增加稳定性
  // 左手offset=170-90=80，右手offset=35-90=-55
  int O[SERVO_COUNT] = {0,
                        0,
                        4,
                        -4,
                        HAND_HOME_POSITION - 90,
                        RIGHT_HAND_HOME_POSITION - 90};
...
void Otto::Bend(int steps, int period, int dir) {
  ...
  int bend1[SERVO_COUNT] = {
      90, 90, 65, 45, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  int bend2[SERVO_COUNT] = {
      90, 90, 62, 145, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  int homes[SERVO_COUNT] = {
      90, 90, 90, 90, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
...
void Otto::ShakeLeg(int steps, int period, int dir) {
  ...
  int shake_leg1[SERVO_COUNT] = {
      90, 90, 70, 50, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  int shake_leg2[SERVO_COUNT] = {
      90, 90, 70, 110, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  int shake_leg3[SERVO_COUNT] = {
      90, 90, 70, 80, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  int homes[SERVO_COUNT] = {
      90, 90, 90, 90, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
...
void Otto::LookAround(int period, int dir) {
  ...
  if (dir == 1) {
    ...
    look_pose[5] = RIGHT_HAND_HOME_POSITION;
  } else {
    // 向右看：两腿向右转，左脚支撑，右脚微抬
...
void Otto::UpDown(float steps, int period, int height) {
  ...
  int O[SERVO_COUNT] = {
      0, 0, height, -height, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
  double phase_diff[SERVO_COUNT] = {0, 0, DEG2RAD(-90), DEG2RAD(90), 0, 0};
...
void Otto::Jitter(float steps, int period, int height) {
  ...
  int O[SERVO_COUNT] = {
      0, 0, 0, 0, HAND_HOME_POSITION, RIGHT_HAND_HOME_POSITION};
...
void Otto::HandsUp(int period, int dir) {
  ...
  // 左手放下=170°，右手放下=35°
  int target[SERVO_COUNT] = {90, 90, 90, 90, HAND_HOME_POSITION, 35};

  // 左手抬起：45度
  // 右手抬起：180度
  if (dir == 0) {
    target[LEFT_HAND] = 45;   // 左手抬起
    target[RIGHT_HAND] = 180; // 右手抬起
  } else if (dir == 1) {
    target[LEFT_HAND] = 45;
...
void Otto::HandWave(int period, int dir) {
  ...
  if (servo_index == LEFT_HAND) {
    current_positions[servo_index] = HAND_HOME_POSITION;  // 左手放下：170°
  } else {
    current_positions[servo_index] = 35;  // 右手放下：35°
  }
  MoveServos(300, current_positions);
}
...
void Otto::HandWaveBoth(int period) {
  ...
  current_positions[LEFT_HAND] = HAND_HOME_POSITION;  // 左手放下：170°
  current_positions[RIGHT_HAND] = 35;                 // 右手放下：35°
  MoveServos(300, current_positions);
}
...
void Otto::HandWaveSmooth(int period, int dir) {
  ...
  int home_pos = (servo_index == LEFT_HAND) ? HAND_HOME_POSITION
                                            : RIGHT_HAND_HOME_POSITION;
  int up_pos = (servo_index == LEFT_HAND) ? 45 : 180;
